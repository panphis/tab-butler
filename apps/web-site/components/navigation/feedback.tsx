...
export default function RightSideBar({ slug, title }: SideBarEdit) {
  ...
  return (
    <div className="flex flex-col gap-3 pl-2">
      <h3 className="text-sm font-semibold">关于此页</h3>
      <div className="flex flex-col gap-2">
        ...
        >
          <LuArrowUpRight className="mr-1 w-4 h-4 inline-block" /> 反馈
        </Link>
        <Link
          href={editUrl}
          target="_blank"
        ...
        >
          <LuArrowUpRight className="mr-1 w-4 h-4 inline-block" /> 编辑此页
        </Link>
      </div>
    </div>
  )
